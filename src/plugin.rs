...
fn movement(
    ...
    for event in movement_event_reader.read() {
        for (movement_acceleration, jump_impulse, mut linear_velocity, transform, is_grounded) in
            &mut controllers
        {
            let mut local_linear_velocity = Vector::default();
            ...
            match event {
                MovementAction::Move(direction) => {
                    local_linear_velocity.x += direction.x * acceleration * delta_time;
                    local_linear_velocity.z -= direction.y * acceleration * delta_time;
                }
                MovementAction::Jump => {
                    if is_grounded {
...
/// Applies friction to character controllers.
fn apply_friction(
    time: Res<Time>,
    mut controllers: Query<
        (&mut LinearVelocity, &Transform, &MovementAcceleration),
    ...
    for (mut linear_velocity, transform, movement_acceleration) in &mut controllers {
        let horizontal_velocity = linear_velocity.0
            + linear_velocity.reject_from_normalized(transform.rotation.mul_vec3(Vector::NEG_Y));
        linear_velocity.0 -=
            horizontal_velocity.normalize() * delta_time * (movement_acceleration.0 / 2.0);
    }
}
